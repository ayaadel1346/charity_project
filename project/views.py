...
def cancel_project(request, project_id):
    ...
    if project.creator != request.user:
        return HttpResponseForbidden("You do not have permission to cancel this project.")
    
    total_donations = project.total_donations()
    target_amount = float(project.total_target)
    if total_donations >= 0.25 * target_amount:
        return render(request, 'viewPage/error.html')
    else:
        project.total_target -= 10   
...
def app_view(request):
    return render(request, "viewPage/viewProject.html", status=200)  
...
def payment_successful(request):
    ...
    donation.save()
    return render(request, 'viewPage/viewProject.html', {'Donation':donation})    
